...
{		
    ...
    vec4 TangentFragPos = vec4(TBNInverse * fs_in.FragPos.xyz, 1.0);
    vec4 TangentCamPos = vec4(TBNInverse * camPos.xyz, 1.0);
    ...
    for(int i = 0; i < 2; ++i) 
    {
        ...
        else            //Point light
        {
            ...
            H = normalize(V + L);
            float dist = length(lightPositions[i] - fs_in.FragPos.xyz);
            //float attenuation = 1.0 / dist * dist;
            float attenuation = clamp(1.0 - dist * dist / 100, 0.0, 1.0);
            radiance = lightColors[i] * 5.0 * attenuation;
        }
